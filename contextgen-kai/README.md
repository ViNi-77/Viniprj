...
**資料を整理・確認してCopilotへ渡す、ローカル専用アプリ。開発版 0.3.0。**
...
Builder向けTXTとStudio向けMarkdownを用途別に準備します。複数フォルダの案件セット、整理前後と単位ごとの採否、ページや表見出しを保った出力、混在PDF・Excel保存値の読み取り、警告資料の再読、投入済みファイルを基準にした差分、任意の評価質問CSVを追加しました。AI・外部APIへ接続せず、Copilotへの登録は利用者が行います。

## 配布者と利用者の入口

このリポジトリは配布者・開発者が管理するソース一式です。利用者には、Windowsでビルドした **配布ZIPだけ** を渡します。GitのクローンやPythonの準備は利用者に依頼しません。

- 利用者: ZIPを「すべて展開」→ `ContextgenKai.exe` を起動。操作説明は [画像付きマニュアル](contextgen改_操作マニュアル.html)。
- 配布者: 以下のGit取得・更新手順でソースとOCRを一緒に管理し、成功したWindowsビルドからZIPを取得。
- 設計3文書・検証記録・撮影記録はこのソース側に保管し、利用者向けZIPには含めません。
- 配布情報の取捨選択とコード保護の判断材料は [配布情報整理](docs/05_配布情報整理とソース保護.md) を参照。

## 配布者がWindowsでGitから取得する
...
0.3.0のWindows成果物は、該当版のCI成功を確認してから配布してください。旧版の試験結果は新版の受入に代用しません。
...
更新後は `起動.bat` を実行してください。依存の定義が変わった場合だけ再準備し、設定・本文修正・出力世代は利用者専用領域に保持します。画面左下の版表示が **0.3.0** になったことを確認します。

## 配布者がZIPを準備する

[Windows CI](https://github.com/ViNi-77/Viniprj/actions/workflows/contextgen-kai.yml)の成功した実行から `contextgen-kai-windows` 成果物を取得し、中の `contextgen-kai-0.3.0-windows-x64.zip` を利用者へ配布します。GitHub成果物の外側のZIPと、配布用ZIPを取り違えないでください。配布先PCでの最終受入は別途必要です。

利用者に渡すZIPは次の4点をルートに置きます。依存関係・OCR・画像・権利表示は `_internal` にまとめ、フォルダ全体を維持します。
...
└─ _internal/
   ├─ ocr/                       # OCR本体・日本語/英語
   ├─ manual/images/             # 操作画像・表示用JS
   ├─ THIRD_PARTY_LICENSES/       # 第三者の権利表示
   └─ …                          # アプリ実行資源
```

更新も配布者が新しいZIPを渡します。利用者はバックアップ・予約停止・アプリ終了後、新ZIPを別フォルダに展開して起動します。設定と修正は利用者領域に残ります。実行フォルダが変わるため、背景予約は新しいアプリで保存し直してから有効にします。古いZIPフォルダ内へ上書き展開せず、新版の動作を確認してから旧フォルダを整理します。

## ファイル構成

以下は配布者が保持するソースの構成です。写真に合わせ、設計文書・マニュアル・ビルド入口を直下に置いています。利用者向けZIPの構成とは分けています。
...
新規部分は独立実装で、旧contextgenのソース・既存の権利表示を変更していません。新しい権利帰属・ライセンスの宣言はしていません。OCRのライセンス・ビルド出典は `ocr/`、EXE配布の依存ライセンスは `_internal/THIRD_PARTY_LICENSES/` に含めます。
