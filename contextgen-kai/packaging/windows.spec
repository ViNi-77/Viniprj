...
for package in ('pypdfium2', 'pypdfium2_raw'):
    data, binaries, imports = collect_all(package, include_py_files=False)
    extra_datas += data
...
a = Analysis([str(root / 'launch.py')], pathex=[str(root)],
             ...
             hiddenimports=hidden, hookspath=[], runtime_hooks=[], excludes=['pytest', '_pytest', 'playwright'], noarchive=False)
pyz = PYZ(a.pure)
