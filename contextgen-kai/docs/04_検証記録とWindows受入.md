...
- macOS26.6.2 arm64 / Python3.12.13: 自動試験58件成功、Windowsジャンクション1件のみ対象OS外としてスキップ。実際の英語・日本語OCR、混在PDF、Office内画像も成功。
...
- 停止・再開、修正競合、容量不足、原本範囲外へのリンク差替、バックアップ内の不正ID、出力の世代保護を回帰試験で確認。
...
Windows CIと利用予定PCの実機受入は、実績を得てから追記する。1.0.0へは昇格していない。
