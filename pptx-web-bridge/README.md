...
| 色の再現（Phase K） | 取り込んだ PowerPoint の色を、PowerPoint と同じ色で再現する。色パレットの「明るく / 暗く」（lumMod / lumOff）は HSL の輝度、shade / tint はリニアガンマ空間で計算し、satMod / hueMod にも対応する。これまでは python-pptx 任せで、「暗く 25%」が 1 色ずれ、shade / tint 付きの塗りは変換前の色のまま出ていた | pytest 190、E2E 52、UI スモーク 47 |
| 元と見比べる（Phase L） | テンプレート推定の画面に**元のスライドを並べて出す**。従来は「このアプリの解釈」だけを番号付きで断言しており、ロゴが落ちても理由が分からなかった。あわせて黙って失敗する経路を塞いだ: 1 枚だけの PPTX で部品と警告が消える／色を取れない図形（グラデーション・ベクター）を無警告で捨てる／EMF・WMF のロゴが枠ごと消える／色見本の判定が帯を巻き込む／ロゴ + タグラインの横長ロックアップ（幅 25% 超）を拾えない。薄い帯がリサイズハンドルに覆われて掴めない不具合も直した | pytest 206、E2E 52、UI スモーク 49 |
| Copilot の用途を明示（Phase M） | 「何の役に立つか分からない」を直す。ダイアログの先頭に**何が出来上がるか**と 3 手順（今どこにいるか付き）を出し、用途ごとに出来上がりの実例を添える。渡し方のボタンは「全部コピー」と「Copilot を開く」の 2 つに絞り、残りは「その他の渡し方」に畳む。Copilot Studio に登録するエージェント一式は別の入口へ移した。あわせて**解析の失敗を黙らせない**: 知らない `型:`・行頭でない `ノート:`・見出し数の不一致・番号なしノートの位置合わせを警告に出す（従来は無言でふつうのスライドになり、前置き 1 行で全ノートが 1 枚ずつずれていた） | pytest 211、E2E 52、UI スモーク 52 |
...
画面右上に **「版 0.5.3 (コミット)」** のバッジが出ます。押すと、その版に入っている機能が段階ごとに一覧で出ます。
...
| 版 | 入っているもの |
|---|---|
| 0.5.3 | Phase M（Copilot の用途を明示: 何が出来上がるかを先に出し、読み取れなかった型・ノートを黙って通さない） |
| 0.5.2 | Phase L（元と見比べる: テンプレート推定の隣に元のスライドを出し、落ちた部品と理由を必ず表示する） |
| 0.5.1 | Phase K（色の再現: PowerPoint の「明るく / 暗く」・shade / tint を同じ色で取り込む） |
...
7. **Copilot で下書きを作る（API 不使用）**: 「Copilot で下書きを作る」で何を作るかを選び「全部コピー」→ M365 Copilot チャットに貼る → 回答をコピー → 「回答を貼り付けて反映」。PowerPoint で作らせるときは「Word 文書を保存」して Copilot in PowerPoint の「ファイルから作成」に使い、ブランドキットを適用する。できた PowerPoint はそのまま「ファイル投入」で取り込んで Web 化できる。資料の文章をそのまま Copilot に貼るため、社外秘の資料は社内テナントの M365 Copilot でのみ使うこと（外部の AI サービスには貼らない）。
8. **Copilot エージェント一式（任意）**: 「Copilot で下書きを作る」→「その他の渡し方」→「Copilot 側に書式を覚えさせる」→「一式を ZIP で保存」。Copilot Studio で新しいエージェントを作り、`instructions.md` の本文を指示に貼り、`knowledge/` のテキストをナレッジとして添付する（M365 Copilot は 20 ファイルまで）。以後はそのエージェントに貼るだけで、前置き無しの Markdown が返る。ここでも社外秘の資料は社内テナントの Copilot でのみ扱うこと。
